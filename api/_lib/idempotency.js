      ...
      `${field} must be a real unique message identifier, not the user's confirmation text.`,
      {
        ...
        recommended_format:
          field === "X-Idempotency-Key"
            ? `nimble:<unique_client_request_id>:${action || "action"}`
            : "Use a real unique technical identifier generated by the integration for this request.",
      }
    );
  }
...
function validateIdempotencyIdentifiers({
  ...
  const rawProviderMessageId = String(providerMessageId ?? "").trim();

  const provider = rawProviderMessageId
    ? validateIdentifier(rawProviderMessageId, {
...
async function reserveCommand({
  ...
  const { error } = await supabase.from(IDEMPOTENCY_TABLE).insert({
    user_id: userId,
    provider_message_id: providerMessageId || idempotencyKey,
